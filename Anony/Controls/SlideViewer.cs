...
namespace Anony.Controls
{
    ...
    public class SlideViewer:ItemsControl
    {
        ...
        public static readonly DependencyProperty SelectedIndexProperty =
            DependencyProperty.Register("SelectedIndex", typeof(int), typeof(SlideViewer),
                new PropertyMetadata(0, new PropertyChangedCallback(OnSelectedIndexChanged)));

        private static void OnSelectedIndexChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ((SlideViewer)d).OnSelectedIndexChanged(e);
        }
        ...
        public static readonly DependencyProperty IsSlideEnabledProperty =
            DependencyProperty.Register("IsSlideEnabled", typeof(bool), typeof(SlideViewer),
                new PropertyMetadata(true));

        #endregion

        internal SlideViewerPanel Panel { get; set; }
        ...
        public SlideViewer()
        {
            DefaultStyleKey = typeof(SlideViewer);
            ...
            CoreWindow.GetForCurrentThread().PointerPressed += SlideViewer_PointerPressed;
        }

        void SlideViewer_PointerPressed(CoreWindow sender, PointerEventArgs args)
        {
            try
            {
                if ((Window.Current.Content as Frame).CurrentSourcePageType == typeof (HomeView))
                {
                    var ps = args.GetIntermediatePoints();
                    if (ps != null && ps.Count > 0)
                    {
                        recognizer.ProcessDownEvent(ps[0]);
                        //args.Handled = true;
                    }
                }
            }
        ...
        private void OnPointerReleased(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (ps != null && ps.Count > 0)
            {
                recognizer.ProcessUpEvent(ps[0]);
                e.Handled = true;
        ...
        void recognizer_ManipulationCompleted(GestureRecognizer sender, ManipulationCompletedEventArgs args)
        {
            var finalVelocities = args.Velocities.Linear;
            CompleteManipulation(finalVelocities);
        }

        void recognizer_ManipulationUpdated(GestureRecognizer sender, ManipulationUpdatedEventArgs args)
        {
            if (!IsSlideEnabled)
            {
                return;
            }
            var deltaManipulation = args.Delta;
        ...
        void recognizer_ManipulationStarted(GestureRecognizer sender, ManipulationStartedEventArgs args)
        {
 ...
 override void OnApplyTemplate()
        ...
        {
#if WINDOWS_PHONE
            var appSize = new Size(Application.Current.Host.Content.ActualWidth,
...
#if WINDOWS_PHONE
        private void SlideView_ManipulationDelta(object sender, ManipulationDeltaEventArgs e)
#elif NETFX_CORE
...
#if WINDOWS_PHONE
        private void SlideView_ManipulationCompleted(object sender,
...
#if WINDOWS_PHONE
        private void SidePanelStoryboard_Completed(object sender, EventArgs e)
#elif NETFX_CORE
